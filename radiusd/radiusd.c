...
pth_mutex_t request_list_mutex;

#define request_list_block() \
 pth_mutex_acquire(&request_list_mutex, FALSE, NULL);
#define request_list_unblock() \
 pth_mutex_release(&request_list_mutex)
...
pth_t radius_tid; /* The PID of the main process */
	...
	char **argv;
{
	...
	if (!pth_mutex_init(&request_list_mutex)) {
		radlog(L_CRIT|L_PERROR, "pth_mutex_init");
		exit(1);
	}

	...
	int *errp;
{
	...
	/* first, find the node itself */
 	rc = mib_lookup(node, oid, OIDLEN(oid), &node);
	...
	*varp = temp_var;
	(*node->handler)(VAR_SUBID_GET, node->closure,
...
void get_nasstat(NAS *nas, struct snmp_var *var, int ind);

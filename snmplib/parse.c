     ...
     struct node **nodes;
{
	...
	tp = (struct snmp_mib_tree *) emalloc(sizeof(struct snmp_mib_tree));
	tp->parent = root;
	tp->next_peer = NULL;
	tp->child_list = NULL;
     ...
     int length;		/* the length of the array */
{
	...
	if (type == LABEL) {
	    ...
	    cp = (char *) emalloc((unsigned) strlen(token) + 1);
	    strcpy(cp, token);
	    SubOid->label = cp;
	    type = get_token(fp, token);
	    if (type == LEFTPAREN) {
     ...
     char *name;
{
	...
	np = root = (struct node *) emalloc(sizeof(struct node));
	...
	for (count = 0, op = SubOid, nop = SubOid + 1; count < (length - 2); count++,
		...
		np->enums = 0;
		/* set up next entry */
		np->next = (struct node *) emalloc(sizeof(*np->next));
	...
	if (nexttype == LEFTBRACKET) {
		...
		if (type == LABEL) {
			...
			ep = np->enums = (struct enum_list *)
			    emalloc(sizeof(struct enum_list));
		    } else {
		    ...
		    ep->label = (char *) emalloc((unsigned) strlen(token) + 1);

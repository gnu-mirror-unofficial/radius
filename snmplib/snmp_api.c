...
snmp_open(struct snmp_session *session)
{
    ...
    slp = (struct session_list *) emalloc(sizeof(struct session_list));
    if (slp == NULL) {
	snmp_set_api_error(SNMPERR_OS_ERR);
	return (NULL);
    }
    /* Internal session */
    isp = (struct snmp_internal_session *) emalloc(sizeof(struct snmp_internal_session));
    if (isp == NULL) {
	efree(slp);
	snmp_set_api_error(SNMPERR_OS_ERR);
	return (NULL);
    ...
    /* The actual session */
    slp->session = (struct snmp_session *) emalloc(sizeof(struct snmp_session));
    if (slp->session == NULL) {
	efree(isp);
	efree(slp);
	snmp_set_api_error(SNMPERR_OS_ERR);
	...
	cp = (u_char *) emalloc((unsigned) strlen(session->peername) + 1);
	if (cp == NULL) {
	    efree(slp->session);
	    efree(isp);
	    efree(slp);
	...
	cp = (u_char *) emalloc((unsigned) session->community_len);
	if (cp)
...
snmp_close(struct snmp_session *session)
{
	...
	if (slp->session->community != NULL)
	    efree((char *) slp->session->community);
	if (slp->session->peername != NULL)
	    efree((char *) slp->session->peername);
	efree((char *) slp->session);
	if (slp->internal->sd != -1)
	    close(slp->internal->sd);
	free_request_list(slp->internal->requests);
	efree((char *) slp->internal);
	efree((char *) slp);
    } else {
    ...
    int length)
{
    ...
    bufp = (u_char *) emalloc(CommunityLen + 1);
    if (bufp == NULL)
	return (NULL);
...
snmp_send(struct snmp_session *session, struct snmp_pdu *pdu)
{
	...
	if (pdu->enterprise_length == SNMP_DEFAULT_ENTERPRISE_LENGTH) {
	    pdu->enterprise = (oid *) emalloc(sizeof(DEFAULT_ENTERPRISE));
	...
	rp = (struct request_list *) emalloc(sizeof(struct request_list));

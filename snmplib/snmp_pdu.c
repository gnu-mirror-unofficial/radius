...
snmp_pdu_create(int command)
{
    ...
    pdu = (struct snmp_pdu *) emalloc(sizeof(struct snmp_pdu));
    if (pdu == NULL) {
...
snmp_pdu_clone(struct snmp_pdu *Src)
{
    ...
    Dest = (struct snmp_pdu *) emalloc(sizeof(struct snmp_pdu));
    if (Dest == NULL) {
	snmp_set_api_error(SNMPERR_OS_ERR);
	return (NULL);
    }
...
{				/* pdu */
	...
	PDU->enterprise = (oid *) emalloc(PDU->enterprise_length * sizeof(oid));

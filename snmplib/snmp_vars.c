...
snmp_var_new(oid * Name, int Len)
{
    ...
    New = (struct variable_list *) emalloc(sizeof(struct variable_list));
    if (New == NULL) {
    ...
    New->name = (oid *) emalloc(Len * sizeof(oid));
    if (New->name == NULL) {
	efree(New);
	snmp_set_api_error(SNMPERR_OS_ERR);
	return (NULL);
...
snmp_var_clone(struct variable_list *Src)
{
    ...
    Dest = (struct variable_list *) emalloc(sizeof(struct variable_list));
    if (Dest == NULL) {
	snmp_set_api_error(SNMPERR_OS_ERR);
	return (NULL);
    }
    ...
    if (Src->name != NULL) {
	Dest->name = (oid *) emalloc(Src->name_length * sizeof(oid));
	if (Dest->name == NULL) {
	    snmp_set_api_error(SNMPERR_OS_ERR);
	    efree(Dest);
	    return (NULL);
	...
	(Src->val_len)) {
	Dest->val.string = (u_char *) emalloc(Src->val_len);
...
snmp_var_free(struct variable_list *Ptr)
{
	...
	efree((char *) Ptr->val.string);
    else if (Ptr->val.integer && Ptr->val_len > 0)
	efree((char *) Ptr->val.integer);
    ...
    int Version)
{
	...
	case ASN_INTEGER:
	    Var->val.integer = (int *) emalloc(sizeof(int));
	...
	case SMI_GAUGE32:
	    /*  case SMI_UNSIGNED32: */
	case SMI_TIMETICKS:
	    Var->val.integer = (int *) emalloc(sizeof(u_int));
	    if (Var->val.integer == NULL) {
		snmp_set_api_error(SNMPERR_OS_ERR);
		return (NULL);
	    }
	...
	case SMI_OPAQUE:
	    ...
	    Var->val.string = (u_char *) emalloc((unsigned) Var->val_len);
	...
	case ASN_OBJECT_ID:
	    ...
	    Var->val.objid = (oid *) emalloc((unsigned) Var->val_len);
	    if (Var->val.integer == NULL) {
		snmp_set_api_error(SNMPERR_OS_ERR);
		return (NULL);
	    }
